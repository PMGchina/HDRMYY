...
var enableGotoCurrentPage = false;
...
function initPageableData(page, async, size, sort, direction) {
  ...
  $.ajax({
    ...
    async: async,
    type: 'POST',
    url: generateFullRequestPath(loadPageableDataUrl),
...
function isAccessCurrentPage(page, async) {
	return enableGotoCurrentPage ? false : (async && page == getCurrentPageNumber());
}
...
//          html += '<li id="' + (start - 1) + 'li" class="am-active"><a href="javascript:"><span>' + start + '</span></a></li>';
        	html += '<li id="' + (start - 1) + 'li" class="am-active"><a href="javascript:initPageableData('
              + (start - 1) + ',true)"><span>' + start + '</span></a></li>';
        } else {
