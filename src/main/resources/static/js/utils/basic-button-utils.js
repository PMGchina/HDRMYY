...
$('button#addButton').click(function(){
	...
	popModal('add',addModalWidth,addModalHeight);
});
$('button#updateButton').click(function(){
	...
	if (count == 0) {
		...
		if (doFunctionWithSingleData(initUpdateForm,updateBefore)) 
			popModal('update',updateModalWidth,updateModalHeight);
	} else {
...
var popModal = function(target,width,height) {
	var options = {closeViaDimmer: 0};
	if (target != null) {
		if (width) options.width = width;
		if (height) options.height = height;
		$('#doc-modal-'+target).modal(options);
	}
}
...
$('button#deleteButton').click(function() {
	...
	//删除前，执行需要在删除执行前执行的方法
	if ((deleteBefore && deleteBefore(ids)) || !deleteBefore) {
		ask('您确定要删除选中的' + count + '条记录吗？','是','否',function(){
			//执行删除操作
			$.del(generateFullRequestPath('delete'),ids,function(){
				warn('删除成功!');
...
var doFunctionWithSingleData = function(initFormFn,actionBeforeFn) {
	...
	$.ajax({
        ...
        async: false,
	    type: 'POST',
	    url: generateFullRequestPath('single'),
