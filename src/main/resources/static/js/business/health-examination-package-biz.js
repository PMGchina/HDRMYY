moduleName = 'healthExaminationPackage';
addModalWidth = updateModalWidth = copyModalWidth = 800;
var multiselectOptionsCache;
...
var columnNames = ['ID','套餐名','总价','可用状态','操作'];
//tableContainer中表格每列对应的业务模型实体类的属性名
var attributeNames = ['id','name','amount','available','operation'];
//tableContainer中表格每列需要的按钮
var buttonsOnEachRow = ['update?','copy?','delete?'];
//提交add、update或copy表单时的表单验证规则
var validateRules = [
	{name: 'name', required: true, minLength: 2, maxLength: 100},
	{name: 'amount', required: true, greatEqual: '0.00'},
	{name: 'description', maxLength: 512}
];
/*****************************************************************************************
 *************************************** LIST PAGE ***************************************
 *****************************************************************************************/
loadPageableDataUrl = 'healthExaminationPackagesByPage'; //加载分页数据对应的后台请求映射URL
...
documentReady = function() { //pagination-utils.js 绑定所有表单单击(目的为提交表单)事件
	...
	initMultiselectOptions(actions.add.key);
};

function initMultiselectOptions(formKey) {
	var $select = $('#'+formKey+'_from');
	var content = [];
	if (!multiselectOptionsCache || multiselectOptionsCache.length == 0) {
		$.gety(projectPath + '/admin/healthExaminationItem/all4select',function(data) {
			multiselectOptionsCache = data.all;
		});
	}

	for(var r in multiselectOptionsCache) {
		var item = multiselectOptionsCache[r];
	...
	$select.empty();
	$select.append(content.join('')).multiselect({
		right: '#'+formKey+'_items',
		rightSelected: '#'+formKey+'_multiselect_rightSelected',
		rightAll: '#'+formKey+'_multiselect_rightAll',
		leftSelected: '#'+formKey+'_multiselect_leftSelected',
		leftAll: '#'+formKey+'_multiselect_leftAll',
		moveUp: '#'+formKey+'_multiselect_move_up',
		moveDown: '#'+formKey+'_multiselect_move_down',
		sort: false
	});
}
...
function saveData(sucFunc) {
	...
	//提交表单时，携带体检项目id数组
	var array = new Array();
	$('#'+currentAction.key+'_items option').each(function() {
		var value = $(this).val();
		if (value && value.trim().length > 0) {
			array.push(Number(value));
		}
	});
	savingFormData.items = array;
...
function initForm(formKey,obj) {
	...
	$('#'+formKey+'_amount').val(obj.amount);
	
	CKEDITOR.instances[formKey + '_description'].setData(obj.description);
	$('#'+formKey+'_description').val(obj.description);
	
	var $items = $('#'+formKey+'_items');
	$items.empty();
	initMultiselectOptions(formKey);
	$('#'+formKey+'_from option').each(function() {
		if ($.inArray(Number($(this).val()), obj.items) >= 0) {
			$items.append($(this));
		}
	});
