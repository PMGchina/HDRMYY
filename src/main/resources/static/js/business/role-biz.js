...
documentReady = function () { //pagination-utils.js 绑定所有表单单击(目的为提交表单)事件
  $('#submitAdd').click(function () {
    ...
    submitForm(validateForm(currentAction.key, validateRules), currentAction.key, function () {
      $('#cancelAdd').click();
    });
  });
  ...
  $('#submitAllot').click(function () {
    ...
    submitForm(true, currentAction.key, function () {
      $('#closeAllot').click();
    });
  });
...
function refreshMappingButtonClickFunction() {
  $.call(generateFullRequestPath('refreshMapping'));
  warn('更新成功!');
}
...
function allotPermissionButtonClickFunction() {
  ...
  if (count == 0) {
    ...
    popModal(actions.allot.key,allotModalWindoWidth,allotModalWindowHeight)
  } else {
    warn('系统暂不支持批量分配权限功能！');
  }
}
...
initUpdateForm = function (obj) {
  initForm(actions.update.key, obj)
} //basic-button-utils.js
initCopyForm = function (obj) {
  initForm(actions.copy.key, obj)
} //basic-button-utils.js
initAllotForm = function (obj) {
  initForm(actions.allot.key, obj)
} //in current JavaScript file
...
function fresh4NewData(data, callback) {
  if (data != null) {
    if (currentAction == actions.allot) {
      if (data.allotted == false)
        warn('分配角色权限失败，请查看系统日志!');
...
function checkUsedRoles(obj) {
  ...
  $.ajax({
    ...
    async: false,
    type: 'POST',
    url: generateFullRequestPath('isUsedRoles'),
    data: obj,
    dataType: 'json',
    success: function (data) {
      isUsed = data.isUsed;
...
function generatePermissionCheckbox0(permissions, element, subGroupId) {
  ...
  for (r in allPermissionData) {
    ...
    //选中当前用户拥有的权限
    if (identifyOwnedPermission(permissions, permission))
...
function checkPermissionGroupCheckbox(container) {
  if (!container)
    container = $('#allot_permission_checkbox_container');
