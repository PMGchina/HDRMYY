...
var columnNames = ['ID','姓名','性别','职称','医生编码','科室编码','可用状态','创建日期','最后修改日期','操作'];
//tableContainer中表格每列对应的业务模型实体类的属性名
var attributeNames = ['id','name','gender','title','doctorCode','departmentCode','available','createDate','modifyDate','operation'];
//tableContainer中表格每列需要的按钮
var buttonsOnEachRow = ['update?','copy?','delete?'];
//提交add、update或copy表单时的表单验证规则
var validateRules = [
	{name: 'name', required: true, minLength: 2, maxLength: 20},
	{name: 'title', minLength: 1, maxLength: 100},
	{name: 'imageUrl', maxLength: 256},
	{name: 'description', maxLength: 5000},
	{name: 'doctorCode', maxLength: 20},
	{name: 'departmentCode', maxLength: 20}
];
/*****************************************************************************************
 *************************************** LIST PAGE ***************************************
 *****************************************************************************************/
loadPageableDataUrl = 'directorsByPage'; //加载分页数据对应的后台请求映射URL
...
function initForm(formKey,obj) {
	...
	$('#'+formKey+'_doctorCode').val(obj.doctorCode);
	...
	$('#'+formKey+'Form').find('input[name=gender][value='+(obj.gender)+']').uCheck('check');
