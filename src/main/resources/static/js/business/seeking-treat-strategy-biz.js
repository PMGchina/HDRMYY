moduleName = 'seekingTreatStrategy';
...
var columnNames = ['ID','攻略栏目','可用状态','创建日期','最后修改日期','操作'];
//tableContainer中表格每列对应的业务模型实体类的属性名
var attributeNames = ['id','item','available','createDate','modifyDate','operation'];
//tableContainer中表格每列需要的按钮
var buttonsOnEachRow = ['update?','copy?','delete?'];
//提交add、update或copy表单时的表单验证规则
var validateRules = [
	{name: 'item', required: true, maxLength: 100},
	{name: 'content', maxLength: 65535},
	{name: 'imageUrl', maxLength: 256}
];
/*****************************************************************************************
 *************************************** LIST PAGE ***************************************
 *****************************************************************************************/
loadPageableDataUrl = 'seekingTreatStrategysByPage'; //加载分页数据对应的后台请求映射URL
...
documentReady = function() { //pagination-utils.js 绑定所有表单单击(目的为提交表单)事件
	$('#submitAdd').click(function() {
		...
		$('#add_content').val(CKEDITOR.instances.add_content.getData());
	...
	$('#submitUpdate').click(function() {
		...
		$('#update_content').val(CKEDITOR.instances.update_content.getData());
	...
	$('#submitCopy').click(function() {
		...
		$('#copy_content').val(CKEDITOR.instances.copy_content.getData());
	...
	initCKEditor(['add_imageUrl', 'update_imageUrl', 'copy_imageUrl', 'add_content', 'update_content', 'copy_content']);
};
