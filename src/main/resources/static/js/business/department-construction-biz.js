moduleName = 'departmentConstruction';
...
var columnNames = ['ID','名称','科室编号','可用状态','创建日期','最后修改日期','操作'];
//tableContainer中表格每列对应的业务模型实体类的属性名
var attributeNames = ['id','name','departmentCode','available','createDate','modifyDate','operation'];
//tableContainer中表格每列需要的按钮
var buttonsOnEachRow = ['update?','copy?','delete?'];
//提交add、update或copy表单时的表单验证规则
var validateRules = [
	{name: 'name', required: true, minLength: 1, maxLength: 20},
	{name: 'departmentCode', maxLength: 20},
	{name: 'description', maxLength: 5000},
	{name: 'iconUrl', maxLength: 256}
];
/*****************************************************************************************
 *************************************** LIST PAGE ***************************************
 *****************************************************************************************/
loadPageableDataUrl = 'departmentConstructionsByPage'; //加载分页数据对应的后台请求映射URL
...
documentReady = function() { //pagination-utils.js 绑定所有表单单击(目的为提交表单)事件
	$('#submitAdd').click(function() {
		$('#add_iconUrl').val(CKEDITOR.instances.add_iconUrl.getData());
	...
	$('#submitUpdate').click(function() {
		$('#update_iconUrl').val(CKEDITOR.instances.update_iconUrl.getData());
	...
	$('#submitCopy').click(function() {
		$('#copy_iconUrl').val(CKEDITOR.instances.copy_iconUrl.getData());
	...
	initCKEditor(['add_iconUrl', 'update_iconUrl', 'copy_iconUrl', 'add_description', 'update_description', 'copy_description']);
};
...
function initForm(formKey,obj) {
	...
	$('#'+formKey+'_departmentCode').val(obj.departmentCode);
	
	CKEDITOR.instances[formKey + '_iconUrl'].setData(obj.iconUrl);
	$('#'+formKey+'_iconUrl').val(obj.iconUrl);
