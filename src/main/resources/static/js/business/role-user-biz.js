...
function initTreeData(roleId) {
	...
	$.ajax({
	    type: 'POST',
	    url: generateFullRequestPath(loadTreeDataUrl),
	    data: data,
	    dataType: 'json',
...
function fresh4NewData(data,callback,isByUpdate) {
	if (data != null) {
		if (isByUpdate) {
			$('table#dataTable').find('tr#'+data.updated.userId).html(generateDefaultDataTableTd(attributeNames,parseValuesOnEachRow(data.updated)));
		}
	}
	callback();
}
...
var doFunctionWithSingleData = function(fn,id) {
	$.ajax({
        ...
        async: false,
	    type: 'POST',
	    url: generateFullRequestPath('singleUserRoleCombineRole'),
...
function initForm(formKey,obj) {
	...
	var roleId = [];
	$('#'+formKey+'Form').find(':checkbox:checked').each(function() {
		roleId.push($(this).val());
	});
	$('#'+formKey+'_role_id').val(roleId.join(','));
