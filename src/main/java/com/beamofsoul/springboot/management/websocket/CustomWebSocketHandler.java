...
//					log.debug("message {} has been sent to user {}", content, session.getPrincipal().getName());
//				}
//			} catch (Exception e) {
...
//				session.sendMessage(new TextMessage(content));
			...
			list.stream().forEach(
				e -> {
					try {
						...
						log.debug("message {} has been sent to user {}", content, e.getPrincipal().getName());
	...
	public static void sendMessageToUser(@NonNull WebSocketMessage<?> message, @NonNull String username) {
		...
		if (CollectionUtils.isNotBlank(list)) {
			...
			try {
				session.sendMessage(new TextMessage(content));
				log.debug("message {} has been sent to user {}", content, username);
			} catch (IOException e) {
