...
import static com.beamofsoul.springboot.management.util.ConfigurationReader.PROJECT_BASE_REPOSITORY_BATCH_SIZE;
...
import static com.beamofsoul.springboot.management.util.QueryDSLUtils.doDelete;
import static com.beamofsoul.springboot.management.util.QueryDSLUtils.doUpdate;
import static com.beamofsoul.springboot.management.util.QueryDSLUtils.initQuery;
import static com.beamofsoul.springboot.management.util.QueryDSLUtils.newQuery;
...
public class BaseMultielementRepositoryImpl<T,ID extends Serializable> extends QueryDslJpaRepository<T, ID> implements BaseMultielementRepository<T, ID> {
	 ...
	 * @param path 需要修改的字段
	 * @param expression 修改方式表达式
	 * @return long 修改了多少条记录
	 */
	@Override
	public <S> long update(Path<S> path, Expression<S> expression) {
		return doUpdate(entityManager, entityPath, path, expression);
	}
	 ...
	 */
	@Override
	public <S> long update(Path<S> path, Expression<S> expression, Predicate predicate) {
		return doUpdate(entityManager, entityPath, path, expression, predicate);
	}
	 ...
	 * @Description: 返回当前的实体管理器
	 * @return EntityManager 当前的实体管理器
	 */
