...
public class ConfigurationReader {
	...
	public static final String[] SPRING_PROFILES = new String[] {"dev","prod"};
	
	public static final String SERVER_CONTEXT_PATH = "server.contextPath";
	
	//有关使用ckfinder上传图片相关的配置
	public static final String PROJECT_COMPONENT_CKFINDER_UPLOAD_IMAGE_PATH = "project.component.ckfinder.uploadImagePath";
	public static final String PROJECT_COMPONENT_CKFINDER_MAX_IMAGE_SIZE = "project.component.ckfinder.maxImageSize";
	...
	private static Map<String, ?> getConfigurations(String configurationFilePath) {
		if (configMap == null) {
			...
			String profile = asString(getValue(SPRING_PROFILES_ACTIVE));
			Map tempMap = null;
			if (ArrayUtils.contains(SPRING_PROFILES, profile)) {
				load = new Yaml().load(ConfigurationReader.class.getResourceAsStream(getPath("application-" + profile + ".yml")));
				if (load != null) {
					tempMap = ((Map<String, ?>) load);
